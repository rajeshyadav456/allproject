...
class ContactAPI(generics.GenericAPIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        #RA=serializer.is_valid(raise_exception=True)
        RA=serializer.save()
        return Response({'msg':'Thank you for contacting us','status':200})
...
class SoundAnalysisAPI(generics.GenericAPIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            x , sr = librosa.load('static_media/user_audios/'+audio_file.name)
            ...
            d=round(random.uniform(0, 1),1)
            s=round(random.uniform(0, 1),1)
            RA.DrawTime=d
            RA.ShotTime=s
            RA.save()
...
def dashboard(request):
    ...
    dictV['monthly']=len(SubscribedUser.objects.filter(SubscribedDate__month='1'))
    dictV['yearly']=len(SubscribedUser.objects.filter(SubscribedDate__year='2021'))
...
def population_chart(request):
    ...
    for i in range(12):
        data.append(len(SubscribedUser.objects.filter(SubscribedDate__month=(i+1))))
...
def pie_chart(request):
    ...
    labels=['REGISTER','SUBSCRIBE']
    
    # dictV['SUBSCRIBE']=len(SubscriptionPlan.objects.all())
    # dictV['REGISTER']=len(UserProfiles.objects.all())
    
    data.append(len(UserProfiles.objects.all()))
    data.append(len(SubscriptionPlan.objects.all()))
